#!/usr/bin/env python3
"""
validate_examples.py — Run every documented PromQL example against a cluster.

Walks the per-operator directories (<category>/<operator>/README.md), extracts
the fenced ```promql example block, runs it through the Loki-compatible API,
and reports one of:
...
above the example block:
    ...
    <!-- validation: kind=instant -->
    ```promql
    topk(3, sum by (app_kubernetes_io_name) (go_goroutines))
    ```

kind=range_metric metric query via /api/v1/query_range
kind=instant      metric query via /api/v1/query (default for PromQL examples)
...
Usage:
    ...
    python3 validate_examples.py --only counter/json --show-output
"""
...
_BLOCK_RE = re.compile(r"```promql\n(.*?)\n```", re.DOTALL)


def parse_readme(path: Path):
    """Return (query, options) from the first validation marker + promql block."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate documented PromQL examples.")
    ...
    parser.add_argument("--only", help="Limit to a category (parse) or operator (counter/rate)")
    ...
    for path in readmes:
        ...
        if not query:
            print(f"SKIP  {name} (no ```promql block found)")
            counts["SKIP"] += 1
            continue

        kind = opts.get("kind", "instant")
