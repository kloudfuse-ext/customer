#!/usr/bin/env python3
"""
query_metrics.py — Run a PromQL metric query against Kloudfuse.

Uses the Loki-compatible endpoints:
  GET /api/v1/query        (with --instant: one evaluation, vector result)
  GET /api/v1/query_range  (default: evaluation per step, matrix result)

Usage:
    python3 query_metrics.py --instant --query 'topk(3, sum by (app_kubernetes_io_name) (go_goroutines))'
    python3 query_metrics.py --query 'sum(rate(ingester_kafka_batch_length_count[5m]))' --minutes 30 --step 60
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run a PromQL metric query against Kloudfuse.")
    ...
    parser.add_argument("--query", required=True, help="PromQL metric query")
    ...
    if data["resultType"] == "vector":
        ...
        print(f"Unexpected resultType={data['resultType']} — "
              f"unexpected result type.", file=sys.stderr)
        sys.exit(1)
