...
# Loop through each table and run pauseConsumption
for table in $tables; do
    # Remove _REALTIME suffix to get the base table name
    base_table="${table%_REALTIME}"

    echo "Pausing consumption for: $base_table (from $table)"
    curl -X 'POST' \
      "http://localhost:9000/tables/$base_table/pauseConsumption" \
      -H 'accept: application/json' \
      -d ''
    echo ""
    echo "---"
