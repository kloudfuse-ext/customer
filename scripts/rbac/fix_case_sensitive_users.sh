...
# Usage: ./fix_case_sensitive_users.sh [namespace]
...
# This query finds emails that have duplicates when compared case-insensitively
# but are different when compared case-sensitively
...
while read -r lower_email; do
    ...
    for mixed_user in "${mixed_case_users[@]}"; do
        ...
        if [ -n "$groups" ]; then
            ...
            while IFS='|' read -r group_id permission; do
                ...
                if [ -n "$group_id" ]; then
                    ...
                    kubectl exec "$POD_NAME" -n "$NAMESPACE" -- sh -c "PGPASSWORD=\$POSTGRES_PASSWORD psql -U $DB_USER -d $DB_NAME -c \"INSERT INTO group_members (group_id, user_id, permission) VALUES ('$group_id', '$lowercase_id', '$permission') ON CONFLICT (group_id, user_id) DO NOTHING;\""
                fi
            done <<< "$groups"
            
            # Remove mixed-case user from all groups
            echo "      Removing mixed-case user from all groups..."
            kubectl exec "$POD_NAME" -n "$NAMESPACE" -- sh -c "PGPASSWORD=\$POSTGRES_PASSWORD psql -U $DB_USER -d $DB_NAME -c \"DELETE FROM group_members WHERE user_id = '$mixed_id';\""
        else
        ...
        kubectl exec "$POD_NAME" -n "$NAMESPACE" -- sh -c "PGPASSWORD=\$POSTGRES_PASSWORD psql -U $DB_USER -d $DB_NAME -c \"DELETE FROM users WHERE id = '$mixed_id';\""
