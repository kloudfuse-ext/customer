...
# This version doesn't require a CSV file
# Usage: ./fix_users_auto.sh [namespace]
        ...
        if [ -n "$user_with_uid_email" ]; then
            ...
            if [ -n "$user_with_proper_email" ]; then
                ...
                if [ -n "$groups" ]; then
                    ...
                    while IFS='|' read -r group_id permission; do
                        ...
                        if [ -n "$group_id" ]; then
                            ...
                            kubectl exec "$POD_NAME" -n "$NAMESPACE" -- sh -c "PGPASSWORD=\$POSTGRES_PASSWORD psql -U $DB_USER -d $DB_NAME -c \"INSERT INTO group_members (group_id, user_id, permission) VALUES ('$group_id', '$proper_user_id', '$permission') ON CONFLICT (group_id, user_id) DO NOTHING;\""
                        fi
                    done <<< "$groups"
                    
                    # Remove the user from all groups after transfer
                    echo "  Removing user $user_with_uid_email from all groups..."
                    kubectl exec "$POD_NAME" -n "$NAMESPACE" -- sh -c "PGPASSWORD=\$POSTGRES_PASSWORD psql -U $DB_USER -d $DB_NAME -c \"DELETE FROM group_members WHERE user_id = '$user_with_uid_email';\""
                else
                ...
                if [ -n "$GRAFANA_PASSWORD" ]; then
                    ...
                    # First, find the user in Grafana by login
                    echo "    Searching for user in Grafana with login: $uid"
                    
                    # Use a temporary pod with curl to interact with Grafana
                ...
                # Delete from rbacdb
                kubectl exec "$POD_NAME" -n "$NAMESPACE" -- sh -c "PGPASSWORD=\$POSTGRES_PASSWORD psql -U $DB_USER -d $DB_NAME -c \"DELETE FROM users WHERE id = '$user_with_uid_email';\""
            else
